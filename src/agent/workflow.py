...
def create_workflow(
    agent: Any, max_retries: int = 3
):
    from src.agent.agent import DataAnalysisAgent
    """Create the agent workflow graph."""

    def check_validation_results(state: AgentState) -> Literal["continue", "retry", "failed"]:
    ...
    def check_sql_execution(state: AgentState) ->  Literal["retry", "failed", "continue"]:
