...
class DataAnalysisAgent:
    def __init__(self, config_path: Optional[str] = None):
        ...
        if config_path:
            ...
            config_file = Path(os.path.expanduser("~/dev/data_analyzer/config/config.yaml"))
            self.config = self._load_config(config_file)
            
        # Initialize LLM
        self.llm = OpenAI(
            model_name=self.config["agent"]["llm"]["model_name"],
    ...
    def _create_agent_workflow(self) -> Graph:
