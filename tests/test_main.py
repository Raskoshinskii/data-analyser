...
def test_get_active_issue_ids_from_jira_empty_comments(
    mock_jira_client: MagicMock, sample_issue_comments_empty: Dict[str, List]
) -> None:
...
def test_get_active_issue_ids_from_jira_with_bot_last(
    mock_jira_client: MagicMock,
    sample_issue_comments_with_bot_last: Dict[str, List[Dict[str, Any]]],
) -> None:
...
def test_get_active_issue_ids_from_jira_with_user_last(
    mock_jira_client: MagicMock,
    sample_issue_comments_with_user_last: Dict[str, List[Dict[str, Any]]],
) -> None:
...
def test_get_jira_tickets(mock_jira_client: MagicMock) -> None:
    ...
    with patch(
        "main.get_active_issue_ids_from_jira", return_value=["DATA-1", "DATA-2"]
    ):
...
def test_main_function(
    mock_get_jira_tickets: MagicMock,
    mock_get_active_ids: MagicMock,
    mock_agent_class: MagicMock,
    mock_db_client_class: MagicMock,
