...
def mock_jira_client():
    ...
    mock_client.get_active_issues.return_value = [
        {"key": "DATA-1"}, 
        {"key": "DATA-2"}
    ]
    ...
    mock_client.get_comments.return_value = {
        "comments": [
            {"author": {"accountId": "not-the-bot-id"}}
        ]
    }
...
@pytest.fixture
def sample_issue_comments_empty():
    """Sample empty issue comments."""
    return {"comments": []}


@pytest.fixture
def sample_issue_comments_with_bot_last():
...
@pytest.fixture
def sample_issue_comments_with_user_last():
